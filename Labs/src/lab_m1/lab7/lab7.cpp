...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(0.75, 0.85, 0.85));
    }
    ...
    // Render ground
    {
        ...
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.19, 0.8, 0.6));
    }
...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int pos = glGetUniformLocation(shader->GetProgramID(), "eye_position");
    glUniform3fv(pos, 1, glm::value_ptr(eyePosition));
    ...
    float kd = glGetUniformLocation(shader->GetProgramID(), "material_kd");
    glUniform1f(kd, materialKd);

    float ks = glGetUniformLocation(shader->GetProgramID(), "ks");
    glUniform1f(ks, materialKs);

    int col = glGetUniformLocation(shader->GetProgramID(), "object_color");
