...
void main()
{
    ...
    float intensitate = ambient_light + 1/(d*d) * ( diffuse_light + specular_light );

    // TODO(student): Send color light output to fragment shader
    
    vec3 light = vec3(0.94,0.81,0.99);
    color = object_color * ( light * (ambient_light + (1/(d*d)) * ( diffuse_light + specular_light )));
