...
void Lab9::Update(float deltaTimeSeconds)
{
...

    {
        ...
        remix = 1;
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, mapTextures["random"], mapTextures["grass"]);
        remix = 0;
    }
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
  ...
  //  if (remix) {
        glUniform1i(glGetUniformLocation(shader->program, "remix"), remix);
  //  }
    if (texture1)
    {
...
Texture2D* Lab9::CreateRandomTexture(unsigned int width, unsigned int height)
{
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_LINEAR); //optiunea care ofera cea mai buna calitate, tranzitie smooth intre mipmaps 
